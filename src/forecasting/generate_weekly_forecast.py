...
GCS_PRODUCTION_MODEL = "models/production/production_model.pkl"
MODEL_VERSION        = "production_model"


# ── Load production model from GCS ────────────────────────

def load_best_model():
    from google.cloud import storage

    print(f"Loading production model from gs://{GCS_BUCKET}/{GCS_PRODUCTION_MODEL}")
    gcs = storage.Client()
    blob = gcs.bucket(GCS_BUCKET).blob(GCS_PRODUCTION_MODEL)
    tmp_path = "/tmp/production_model.pkl"
